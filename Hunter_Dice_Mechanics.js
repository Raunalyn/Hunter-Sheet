...
// !vtm atr a# r# m#	// Rolling for attributes a# is the number of dice associated with the attribute, r# number of dice associated with Desperation, m# is a dice modifier to apply. Example !vtm atr a2 r2 m3 
// !vtm skill a# r# m#	// Rolling for skills a# is the number of dice associated with the attribute, s# the number of dice for the skill r# number of dice associated with Desperation, m# is a dice modifier to apply. Example !vtm atr a2 s3 r2 m3 
...
var vtmGlobal = {
	...
	diceGraphicsChatSize: hunterCONSTANTS.GRAPHICSIZE.MEDIUM,
	diceTextResult: "",
	diceTextResultLog: "",
	diceGraphicResult: "",
 ...
 * @param {*} type The type (V) Vampire or (H) Desperation.
...
function getDiceImage(type, roll) {
	if (type === "v") {
		switch (roll) {
			case 1:
				return hunterCONSTANTS.HUNTER.ONE;
			case 2:
				return hunterCONSTANTS.HUNTER.TWO;
			case 3:
				return hunterCONSTANTS.HUNTER.THREE;
			case 4:
				return hunterCONSTANTS.HUNTER.FOUR;
			case 5:
				return hunterCONSTANTS.HUNTER.FIVE;
			case 6:
				return hunterCONSTANTS.HUNTER.SIX;
			case 7:
				return hunterCONSTANTS.HUNTER.SEVEN;
			case 8:
				return hunterCONSTANTS.HUNTER.EIGHT;
			case 9:
				return hunterCONSTANTS.HUNTER.NINE;
			case 10:
				return hunterCONSTANTS.HUNTER.TEN;
			default:
				return null;
		}
	} else if (type === "h") {
		switch (roll) {
			case 1:
				return hunterCONSTANTS.DESPERATION.ONE;
			case 2:
				return hunterCONSTANTS.DESPERATION.TWO;
			case 3:
				return hunterCONSTANTS.DESPERATION.THREE;
			case 4:
				return hunterCONSTANTS.DESPERATION.FOUR;
			case 5:
				return hunterCONSTANTS.DESPERATION.FIVE;
			case 6:
				return hunterCONSTANTS.DESPERATION.SIX;
			case 7:
				return hunterCONSTANTS.DESPERATION.SEVEN;
			case 8:
				return hunterCONSTANTS.DESPERATION.EIGHT;
			case 9:
				return hunterCONSTANTS.DESPERATION.NINE;
			case 10:
				return hunterCONSTANTS.DESPERATION.TEN;
			default:
				return null;
		}
	}
...
function processVampireDiceScript(run, dc) {
	...
	var diceTotals = {
		...
		diceTextLog: "Normal" + attackDiceResults.diceTextLog + "Desperation" + defendDiceResults.diceTextLog
	};
	...
	if (vtmGlobal.diceLogChat === true) {
		if (vtmGlobal.diceLogRolledOnOneLine === true) {
			...
			if (vtmGlobal.diceGraphicsChat === true) {
				...
				outputMessage += "{{Desperation=" + defendDiceResults.diceGraphicsLog + endTemplateSection;
			} else {
				outputMessage += "{{Normal=" + attackDiceResults.diceTextLog + endTemplateSection;
				outputMessage += "{{Desperation=" + defendDiceResults.diceTextLog + endTemplateSection;
			}
		}
	}

	let thebeast = '<img src="https://as1.ftcdn.net/v2/jpg/02/44/33/02/500_F_244330245_pBk6AHsZJqwe54D8O64O3mjVsHBzUgTA.jpg" title="Despair" height="20" width="228"/>';
	...
	if (run.rouseStatRoll) {
		if (diceTotals.successScore > 0) {
			...
			outputMessage += "{{Beast=" + '<img src="https://raw.githubusercontent.com/Roll20/roll20-character-sheets/master/vampire-v5/Banners/DesperationGain.png" title="Desperation Gain" height="20" width="228"/>' + endTemplateSection;
...
function addRollDeclarations(diceTotals, outputMessage, endTemplateSection, thebeast) {
	...
	let critBonus = diceTotals.critScore * 2.0;
	...
	if (diceTotals.failScore >= 5) {
		...
		outputMessage += "{{Beast=" + thebeast + endTemplateSection;
		//	outputMessage += "{{BeastTaunt=" + "Feed Me! (Desperation causes you to be distracted)" + endTemplateSection;
	}
...
var processScriptTabs = function (argv, who, dc) {
	...
	switch (script) {
		case hunterCONSTANTS.VTMCOMMAND:
			switch (argv[0]) {
				case "log":
					switch (argv[1]) {
						case "on":
				...
				case "graphics":
					switch (argv[1]) {
						...
						case "off":
							...
							break;
						case "s":
							vtmGlobal.diceGraphicsChatSize = hunterCONSTANTS.GRAPHICSIZE.SMALL;
							break;
						case "m":
							vtmGlobal.diceGraphicsChatSize = hunterCONSTANTS.GRAPHICSIZE.MEDIUM;
							break;
						case "l":
							vtmGlobal.diceGraphicsChatSize = hunterCONSTANTS.GRAPHICSIZE.LARGE;
							break;
					}
					break;
				case "test":
...
function handleSkillRoll(input) {
	log("Atr/Skill Roll");
	log(input);
	let desperation = input.desperation;
	...
	run.redDice = ((dicepool + desperation) - Math.abs(dicepool - desperation)) / 2
...
function calculateVariables(argv, who) {
	...
	for (i = 2; i < argv.length; i++) {
		...
		if (identifier === "a") {
			...
			// Red die. Used for assigning a value directly to Desperation die.
			let value = parseInt(entry.substring(1), 10);
			input.desperation = value;
